...
## 1. Source Code
- [Source code](https://github.com/cwida/ALP/publication/source_code)
...
**This [script](https://github.com/cwida/ALP/publication/script/install_dependencies.sh) can be used to install all necessary dependencies on Ubuntu.**
...
- **[Plotter](https://github.com/cwida/ALP/publication/plotter)**: Collects data and generates all figures in the paper.
...
- **[Table Generator](https://github.com/cwida/ALP/publication/script/generate_tables.py)**: Generates Tables 4 and 7 from the paper, stores the generated tables in Markdown format in [result_dir](https://github.com/cwida/ALP/publication/tables/), and prints the tables to the terminal.

## 5. Tables from Experimental Results
- The master script generates experimental result tables in Markdown format.

## 6. Documentation
- Detailed instructions for compilation, deployment, execution, and script usage are provided in **[BENCHMARKING](#BENCHMARKING)** section.
...
## 7. Master Script
- **[Master script](https://github.com/cwida/ALP/publication/script/master_script.sh)**: Automates experiments, data collection, and workflow management.
    - **Outputs**:
...
You can download the datasets used in our publication [here](https://drive.google.com/drive/folders/167faTwZJjqJMKM9Yc6E7KF5LUbsitxJS?usp=sharing) (complete_binaries.zip). These files are in binary format (64-bit doubles) and were used throughout the benchmarks in our paper.
...
Additionally, inside `data/datasets_transformer.ipynb`, you can find a [Jupyter Notebook script](https://github.com/cwida/ALP/data/datasets_transformer.ipynb) with guidelines to download datasets from their original source and code to convert them to binary format (64-bit doubles). Some datasets may require heavy pre-processing.

### 3. Set the Correct Toolchain File

There are two toolchain files in the [toolchain directory](https://github.com/cwida/ALP/toolchain). Adjust them based on the `clang` and `clang++` versions you are using.

### 4. Run the Benchmark Script

Run the [master_script.sh](https://github.com/cwida/ALP/publication/script/master_script.sh) on the following architectures:
